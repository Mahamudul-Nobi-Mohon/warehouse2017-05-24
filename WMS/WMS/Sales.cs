...
namespace WMS
{
    public partial class Sales : Form
    {
        ...
        public Sales()
        {
            ...
            fillCombo(comboBoxWarehouseName, query12, "LocationMainName", "LocationMainID");
        ...
        private void treeViewPurchaseItemSales_AfterSelect(object sender, TreeViewEventArgs e)
        {
            try
            {
                ...
                //See Currently Stock
                Currently_Stock(textBoxPdoductID.Text, Convert.ToInt32(comboBoxWarehouseName.SelectedValue.ToString()));
        ...
        private void comboBoxWarehouseName_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            fillCombo(comboBoxFloorName, query12, "LocationName", "LocationID");
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            //    string temp_product_code = textBoxProductCategory.Text;

            //    int temp_pro_id = Convert.ToInt32(textBoxPdoductID.Text);
            ...
            //    textBoxProductName.Text = textBoxPdoductID.Text = textBoxUnitType.Text = textBoxProductCategory.Text = "";
            ...
            else if (comboBoxFloorName.Text == "")
            {
            ...
            else
            {
                ...
                if (dataGridViewSales.Rows.Count > 0)
                {
                    ...
                    if (isEdited == 0)
                    {
                        foreach (DataGridViewRow row in dataGridViewSales.Rows)
                        {
                            ...
                            {
                                rowIndex = -1;
                                if (row.Cells["Id"].Value.ToString().Equals(searchValue) && row.Cells["warehouse_name"].Value.ToString().Equals(comboBoxWarehouseName.Text))
                                {
                                    rowIndex = row.Index;
                                    // break;
                                    ...
                                    if (checkStock()>=  sum)
                                    {
                                        ...
                                        newDataRow.Cells[2].Value = comboBoxWarehouseName.Text;
                                        newDataRow.Cells[3].Value = comboBoxFloorName.Text;
                                        newDataRow.Cells[4].Value = sum;
                                        newDataRow.Cells[5].Value = textBoxUnitType.Text;
                                        newDataRow.Cells[6].Value = comboBoxWarehouseName.SelectedValue;
                                        newDataRow.Cells[7].Value = comboBoxFloorName.SelectedValue;
                                        isEdited = 1;
                                        Clear_All_Last();
                                        break;
                                    }
                ...
                if (isEdited != 1)
                {
                    ...
                    {
                        dt.Rows.Add(textBoxPdoductID.Text, textBoxProductName.Text, comboBoxWarehouseName.Text, comboBoxFloorName.Text, textBoxQuantity.Text, textBoxUnitType.Text, comboBoxWarehouseName.SelectedValue, comboBoxFloorName.SelectedValue);
        ...
        {
            SqlConnection connection = new SqlConnection(conStr);
            string query = "SELECT STOCK FROM ProductDetails WHERE ProductID='" + textBoxPdoductID.Text + "' and WarehouseID='" + comboBoxWarehouseName.SelectedValue + "'";
            SqlCommand command = new SqlCommand(query, connection);
            connection.Open();
            SqlDataReader reader = command.ExecuteReader();
       ...
       int PaymentType = 0;
        ...
        private void UpdateProductDetails(int pro_id, double quantity, int wareHouseId, int floorId)
        {
            ...
            string query1 = "IF EXISTS (SELECT * FROM ProductDetails WHERE ProductID ='" + pro_id + "' AND WarehouseID = '" + wareHouseId + "') UPDATE ProductDetails SET stock = ((SELECT Stock FROM ProductDetails WHERE ProductID = '" + pro_id + "' AND WarehouseID = '" + wareHouseId + "')-'" + quantity + "') WHERE ProductID = '" + pro_id + "' AND WarehouseID = '" + wareHouseId + "'";
            SqlCommand command1 = new SqlCommand(query1, connection1);
            connection1.Open();
            command1.ExecuteNonQuery();
            connection1.Close();
        ...
        private void dataGridViewSales_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            comboBoxWarehouseName.Text = dataGridViewSales.SelectedRows[0].Cells[2].Value.ToString();
            comboBoxFloorName.Text = dataGridViewSales.SelectedRows[0].Cells[3].Value.ToString();
            ...
            else
            {
                try
                {
                    ...
                    foreach (DataGridViewRow row in dataGridViewSales.Rows)
                    {
                        ...
                        string query = "INSERT INTO Sales (CompanyID, SalesNo, SalesDate, SalesTime, SalesCustomerID, SalesRemarks, Reference, SalesProductID, SalesPurchasePrice, SalesSalePrice, SalesQuantity, SalesProductDiscount, SalesTotal, SalesCustomerName, SalesSoldBy, SalesReceivedAmount, SalesChangeAmount, SalesVatRate, SalesVatTotal, SalesPuechaseBy, SalesPurchaseByContact, PaymentType)" +
                                                      " VALUES ('" + CompanyID + "', '" + SerialNo + "', '" + textBoxDate.Text + "', '" + textBoxTime.Text + "', '" + id + "', '" + textBoxRemarks.Text + "', '" + textBoxReference.Text + "', '" + prod_id + "', 0, 0, '" + quantity + "',0,0, '" + textBoxCustomerName.Text + "', 'sold by need to change', 0, 0, 0,  0,  '" + textBoxPurchaseBy.Text + "', '" + textBoxPruchaseByContact.Text + "', 0)";
